...
describe("handleProviderCallback", () => {
  ...
  it("should return LoginMatchedUser when existing login matches current user", async () => {
    ...
    expect(result).toEqual({
      type: "LoginMatchedUser",
    });
  });

  it("should return LoginLinkedToOtherUser when existing login does not match current user", async () => {
    userService.findLogin = vi.fn().mockResolvedValue({
      userId: "otherUserId",
      provider: session.provider,
      providerKey: session.externalId,
    ...
    expect(result).toEqual({
      type: "LoginLinkedToOtherUser",
    });
  });

  it("should return AddedLoginForUser when currentUser exists but no existing login", async () => {
    ...
    expect(result).toEqual({
      type: "AddedLoginForUser",
      currentUser,
    });
  });

  it("should return LoginWithoutUser when existing login exists but no currentUser", async () => {
    userService.findLogin = vi.fn().mockResolvedValue({
      userId: "otherUserId",
      provider: session.provider,
      providerKey: session.externalId,
  ...
  it("should return FoundUserProfile when no existing login and profile user found by email", async () => {
    userService.findLogin = vi.fn().mockResolvedValue(null);
    ...
    expect(result).toEqual({
      type: "FoundUserProfile",
      profileUser: currentUser,
    });
  });

  it("should return CreatedNewUser when no existing login and no profile user found by email", async () => {
    userService.findLogin = vi.fn().mockResolvedValue(null);
